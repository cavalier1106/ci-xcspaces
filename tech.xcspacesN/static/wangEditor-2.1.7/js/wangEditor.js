...
_e(function (E, $) {
    ...
    // 用到 w3c range 的函数，如果检测到阅读器不支持 w3c range，则赋值为空函数
...
_e(function (E, $) {
    ...
    E.fn.customCommand = function (e, commandFn, callback) {
        ...
        // 保存选区（无参数，要从阅读器直接获取range信息）
        this.saveSelection();
        // 重新恢复选区（无参数，要取得刚刚从阅读器得到的range信息）
        this.restoreSelection();
...
_e(function (E, $) {
    ...
    E.fn.getSelfOrParentByName = function (elem, selector, check) {
        ...
        if (!matchesSelector) {
            // 如果阅读器本身不支持 matchesSelector 则使用自定义的hook
...
_e(function (E, $) {
    ...
    Txt.fn.bindTabEvent = function () {
        ...
        $txt.on('keydown', function (e) {
            ...
            // 如果阅读器支持 insertHtml 则插入4个空格。如果不支持，就不管了
        ...
        function handle(elem) {
            ...
            if (nodeName === 'div') {
                $.each(elem.childNodes, function () {
                    // elem.childNodes 可获取TEXT空间，而 $elem.children() 就获取不到
                    handle(this);
                });
                return;
            }
...
_e(function (E, $) {
    ...
    // 超时时间
    E.config.uploadTimeout = 2 * 60 * 1000;
    // 用于存储上传回调事件
    ...
    E.createMenu(function (check) {
        ...
        $contentContainer.on('click', 'a[commandValue]', function (e) {
            ...
            img.onerror = function () {
                // 不是图片
                E.log('如果此处有阅读器js报错，请忽略。这只是编辑器试图通过表情 value 下载图片没有成功，对功能无任何影响');
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        editor.ready(function () {
            ...
            // 这里使用异步，为了不妨碍高级阅读器通过点击 $uploadContent 选择文件
            function hidePanel() {
                menu.dropPanel.hide();
            }
...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        // 由于阅读器自身不支持 lineHeight 命令，因此要做一个hook
...
_e(function (E, $) {
    ...
    E.plugin(function () {
        ...
        if (!customUpload) {
            ...
            alert('自定义上传无效，详看阅读器日志console.log');
